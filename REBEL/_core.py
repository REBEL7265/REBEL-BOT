...
from REBEL import ALIVE_NAME
from REBEL import bot 
from REBEL.utils import admin_cmd, load_module, remove_plugin, sudo_cmd
from REBEL.utils import edit_or_reply as eor

DELETE_TIMEOUT = 3
thumb_image_path = "./Resources/REBEL.jpg"
...
async def send(event):
    ...
    the_plugin_file = "./REBEL/plugins/{}.py".format(input_str)
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "REBEL/plugins/",  # pylint:disable=E0602
                )
            )
...
async def unload(event):
    ...
    try:
        ...
        qwe = await eor(event, f"REBELBOT Has Successfully unloaded {shortname}")
    except Exception as e:
        await qwe.edit(
            "REBELBOT has Successfully unloaded {shortname}\n{}".format(shortname, str(e))
        )
...
async def load(event):
    ...
    try:
        ...
        await qwe.edit(
            f"REBELBOT could not load {shortname} because of the following error.\n{str(e)}"
        )
