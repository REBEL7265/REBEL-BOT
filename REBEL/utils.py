...
from REBEL import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, bot
from REBELBOT import xbot
from REBEL.helpers.exceptions import CancelProcess

ENV = bool(os.environ.get("ENV", False))
if ENV:
    from REBEL.uniborgConfig import Config
else:
...
def load_extra(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"REBEL_PLUGS/{shortname}.py")
        name = "REBEL_PLUGS.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = REBEL.utils
        mod.Config = Config
        mod.borg = bot
        mod.edit_or_reply = edit_or_reply
        # support for paperplaneextended
        sys.modules["REBEL.events"] = REBEL.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["REBEL.plugins." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"REBEL/plugins/{shortname}.py")
        name = "REBEL.plugins.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("Successfully imported " + shortname)
...
def load_pro(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"REBEL/plugins/assistant/{shortname}.py")
        name = "REBEL.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("Successfully imported " + shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["REBEL.plugins.assistant." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"REBEL.plugins.{shortname}"
